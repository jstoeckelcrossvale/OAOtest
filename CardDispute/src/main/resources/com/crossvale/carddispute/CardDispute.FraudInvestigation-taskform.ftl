<script type='text/javascript'>
	...
	function taskFormValidator() {
		...
		var myInputs = new Array();
					myInputs[i] = document.getElementById("assignedGroup");
					i++;
		...
		var j=0;
						if(notEmpty(myInputs[j]) && !isAlphanumeric(myInputs[j])) {
							alert("Please enter valid assignedGroup");
							myInputs[j].focus();
							return false;
						}
					j++;
...
<div id="container">
	<div id="header">
		New Process Instance: /CardDispute/src/main/resources/com/crossvale/carddispute.FraudInvestigation
	</div>
	<div id="content">
	    <input type="hidden" name="processId" value="${process.id}"/>
		<fieldset>
            <legend>Process inputs</legend>
                            		<label for="name">assignedGroup</label>
                            		<div class="div_texbox">
                              		<input name="assignedGroup" type="text" class="textbox" id="assignedGroup" value="" />
                            		</div>
